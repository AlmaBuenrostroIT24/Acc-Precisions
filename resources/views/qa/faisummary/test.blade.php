...
(() => {
  ...
  // ===== Helper AJAX robusto (evita que la UI truene si hay 500) =====
  function fetchJson(url, opts = {}) {
    return $.ajax(Object.assign({
      ...
      return null; // <- devolvemos null para que el flujo del front continúe
    });
  }
      ...
      drawCallback: function() {
        ...
        api.rows({ page: 'current' }).every(function() {
          ...
          if (ctx.tableSamplingCache.has(orderId)) {
            ...
            fetchJson(ROUTES.samplingPlan(lotSize, 'Normal')).then(resp => {
              // Acepta diferentes nombres de campo para el tamaño de muestra
              const raw = (resp && (resp.sample_qty ?? resp.sample_size ?? resp.n ?? resp.sampling ?? resp.size)) ?? 0;
              const ipiReq = parseInt(raw, 10) || 0;
